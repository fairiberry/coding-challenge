...
function endQuiz(){
    ...
    var score = (correctAnswers / (answerList.length + questionsUnanswered)) * 100
    document.getElementById("submitScore").addEventListener("click", function(e){
        e.preventDefault()
        var highscores = [];
        var userScore = {
            user:document.getElementById("intials").value,
            score: score
        }
...
function showHighScore() {
    ...
    if (highscores === null){
        highScore.innerHTML = "no previous scores found"
    } else {
        for (var i = 0; i < highscores.length; i++) {
        highScore.textContent = highscores[i].user + " - " + highscores[i].score + "♡"
